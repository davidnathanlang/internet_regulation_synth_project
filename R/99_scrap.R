...
# Pulled from 03_assemble_data.R ------------------------------------------



vpn_search <- state_df %>% 
  mutate(relative_date = lubridate::as_date(date) - lubridate::make_date(2023, 5, 2)) %>%
  filter(keyword == "pornhub") %>%
  mutate(post_treat = if_else(geo == "US-UT" & relative_date > 0, 1, 0)) %>%
  mutate(relative_date = as.double(relative_date))

vpn_search %>% count(state)
vpn_search %>% count(keyword)



ms<-multisynth(hits~post_treat,unit=state,time=relative_date,data=term_df)
summary(ms)
ms
